/**
 * Curated + user-defined prompt presets for the Generate / Style / Inpaint / Enhance tools.
 */

export type PresetTool = "generate" | "style" | "inpaint" | "enhance";
...
const CUSTOM_KEY = "ait_custom_presets_v1";
...
export function saveCustomPresets(presets: PromptPreset[]) {
  localStorage.setItem(CUSTOM_KEY, JSON.stringify(presets));
}

export function upsertCustomPreset(preset: PromptPreset) {
  const all = loadCustomPresets();
  const idx = all.findIndex((p) => p.id === preset.id);
  if (idx >= 0) all[idx] = { ...preset, custom: true };
  else all.unshift({ ...preset, custom: true });
...
export function allPresets(): PromptPreset[] {
  return [...loadCustomPresets(), ...BUILTIN_PRESETS];
}

export function presetsFor(tool: string): PromptPreset[] {
  return allPresets().filter(
    (p) => !p.appliesTo || p.appliesTo.includes(tool as PresetTool),
  );
}
